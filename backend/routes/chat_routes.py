...
try:
    from backend.models import InvestorBalance
except Exception:
...
def _get_user_safe() -> Dict[str, Any]:
    """
    Resolve user from Authorization/session; merge any body hint.
    """
    base: Dict[str, Any] = {}
    try:
        base = get_request_user(request, current_user)
    except TypeError:
        try:
            base = get_request_user(request)
        except TypeError:
            try:
                base = get_request_user(current_user)
            except Exception:
                base = {}
    except Exception:
        base = {}
...
def _strict_self_investor(user: Dict[str, Any]) -> Optional[Investor]:
    """
    Lock to the investor linked to the session's User.id via Investor.account_user_id.
    If missing, try email/full name.
    """
    try:
        uid = user.get("id")
        if uid:
...
def _resolve_user_id_from_profile(profile: Dict[str, Any]) -> Optional[int]:
    """
    BEST-EFFORT: return AppUser.id using (in priority):
      1) profile.id
      2) profile.email or profile.username
      3) full name match (first+last) if unique
      4) linked investor.account_user_id
    """
    # 1) Direct id
    try:
        if profile.get("id"):
...
def _norm_name(s: str) -> str:
    s = (s or "").lower().strip()
    s = re.sub(r"\.(pdf|xlsx|xls|csv|docx?)$", "", s)  # strip common ext
...
def handle_balance_intent(user: Dict[str, Any], message: str, body: Dict[str, Any]) -> Dict[str, Any]:
    inv: Optional[Investor] = _strict_self_investor(user)
    if not inv:
...
def _match_by_keywords(query: str, docs: List[Document]) -> List[Tuple[Document,float]]:
    """Keyword OR matching; any hit returns the doc. Score blends hits + fuzzy closeness."""
...
def handle_file_intent(user: Dict[str, Any], message: str, body: Dict[str, Any]) -> Dict[str, Any]:
    """
    Resolve the *actual* AppUser.id for the logged-in investor using name/email/username,
    then list shared files from DocumentShare and do keyword/fuzzy matching.
    """
    # Resolve a reliable AppUser.id
    target_uid: Optional[int] = _resolve_user_id_from_profile(user)
    _dprint("resolved AppUser.id =", target_uid, "from profile =", {k: user.get(k) for k in ("id","email","username","first_name","last_name","name")})
    ...
    # Pull shared documents
    try:
        docs: List[Document] = _fetch_shared_docs_for_user_id(target_uid)
    except Exception:
        docs = []

    # If none yet, try via linked investor.account_user_id
    if not docs:
...
def handle_calc_intent(user: Dict[str, Any], message: str, body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sys = (
        "You are Clarus. Using CONTEXT.series (chronological investor records), compute requested metrics "
        "(ROI, MOIC, IRR/XIRR). Provide numbers with dates. Be concise."
    )
