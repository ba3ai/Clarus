...
class Investor(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id, "name": self.name, "company_name": self.company_name, "address": self.address,
            "contact_phone": self.contact_phone, "email": self.email, "owner_id": self.owner_id,
            "account_user_id": self.account_user_id, "invitation_id": self.invitation_id,
            "birthdate": self.birthdate, "citizenship": self.citizenship,
            "ssn_tax_id": "***" if self.ssn_tax_id else None,
            "address1": self.address1, "address2": self.address2, "country": self.country,
            "city": self.city, "state": self.state, "zip": self.zip, "avatar_url": self.avatar_url,
            "created_at": self.created_at.isoformat() if self.created_at else None,
