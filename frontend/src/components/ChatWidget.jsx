...
export default function ChatWidget({
  ...
  async function sendMessage(e) {
    ...
    // --- greet-by-name client-side shortcut (no server roundtrip) ---
    const isGreeting = /^(\s*(hi|hello|hey|hola|howdy|yo|good\s+(morning|afternoon|evening))\b[!.]?\s*)$/i.test(text);
    if (isGreeting) {
      const displayName =
        (who?.user?.first_name && who.user.first_name.trim()) ||
        (who?.user?.username && String(who.user.username)) ||
        (who?.user?.email && String(who.user.email).split("@")[0]) ||
