...
export default function Statements({ profiles = [] }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setMe({ ready: true, user: data?.user || null, investor: data?.investor || null });
      } catch {
        if (!alive) return;
  ...
  async function onGenerateQuarter() {
    ...
    setErr("");
    try {
      const payload = { year: Number(year), quarter: Number(quarter), entity_name: entityName, ...(investorId ? { investor_id: investorId } : {}) };
  ...
  async function openDetail(id) {
    ...
    try {
      const res = await api.get(`/api/statements/${id}/pdf`, { responseType: "blob", params: { inline: 1 } });
